...
REF_HYPO = read_file_to_list('/home/kat/Projects/PhD/BARTScore/SUM/files/ref_hypo_prompt.txt')
        ...
        for metric_name in tqdm(metrics):
            ...
            if 'bert_score' in metric_name:
                ...
                try:
                    ...
                    if not os.path.exists(bert_scorer.baseline_path):
                        print("Disabling the rescaling...")
                        bert_scorer = BERTScorer(
                        idf=False,
                        lang="en",
                        device=self.device,
                    ...
                    for sys_name in self.sys_names:
                        ...
                        for doc_id in self.data:
                            self.data[doc_id]['sys_summs'][sys_name]['scores'].update({
                                f'bert_score_{model_type}_p': P[counter],
                                f'bert_score_{model_type}_r': R[counter],
...
def main():
    ...
    METRICS = []
    if args.bert_score:
        if configs.get("bert_score", False):
            bert_scores = [("bert_score", c) for c in configs["bert_score"]]
            METRICS.extend(bert_scores)
        else:
