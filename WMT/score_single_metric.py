...
class Scorer:
    ...
    def score(self, metrics):
        for metric_name in metrics:
            if metric_name == 'bleu':
            ...
            elif 'bert_score' in metric_name:
                import bert_score

                def run_bertscore(mt: list, ref: list, **kwargs):
                ...
                scores_better_p, scores_better_r, scores_better_f = run_bertscore(self.betters, self.refs, **kwargs)
                scores_worse_p, scores_worse_r, scores_worse_f = run_bertscore(self.worses, self.refs, **kwargs)
                print(f'Finished calculating BERTScore {metric_name}, time passed {time.time() - start}s.')
                self.record(scores_better_p, scores_worse_p, f'bert_score_{model_type}_p')
                self.record(scores_better_r, scores_worse_r, f'bert_score_{model_type}_r')
                self.record(scores_better_f, scores_worse_f, f'bert_score_{model_type}_f')
...
def main():
    ...
    # Update: 2022-09-10
    # -----------------------------------------------------------------------
    # Add the ability to specify different backbones for some of the existing
    # metrics.
    # -----------------------------------------------------------------------
    ...
    # Configs will be a dictionary of <str, list[additional hyperparams]>
    # The string will represent the name of the metric, whereas the list
    # of hyperparameters can be anything. For bert-score we define it to
    # be a list w/ different names of backbones.
    configs: dict = load_configs(args.config_file)
    configs: dict = parse_configs(configs)
    ...
    if args.bert_score:
        bert_score_configs = configs.get("bert_score", ["bert_score"])
        METRICS.extend(bert_score_configs)
    if args.bart_score:
